...
def download_training_data(
    ...
    print(
        ...
        f"start={start} | end={end} | timeframe=1Min | feed={feed}"
    )
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Check credentials, feed permissions, and .env configuration."
        ) from exc
    except Exception as exc:
        raise RuntimeError("Failed to download Alpaca bars for an unexpected reason.") from exc
...
def make_env(df: pd.DataFrame):
    """
    Build the alpha-focused environment:
    - hard no-trade zone
    - no 35% sizing actions
    - stronger forced-exit penalties
    - longer cooldown and hold periods
    - asymmetric bull vs bear selectivity
    """
    def _init():
        return TradingEnv(
            data=df,
            ...
            low_confidence_entry_penalty=0.004,
            weak_edge_penalty=0.006,
            forced_exit_penalty=0.020,
            churn_penalty=0.004,
            min_signal_confidence=0.78,
            no_trade_signal_confidence=0.75,
            no_trade_regime_confidence=0.70,
            no_trade_edge_threshold=0.25,
            flat_reward=0.0,
            winner_hold_bonus=0.20,
            loser_hold_penalty=0.35,
            holding_penalty=0.00008,
            transition_holding_penalty=0.0040,
            tqqq_extra_penalty=0.0010,
            sma_gate_penalty=0.004,
            tqqq_bull_strength_threshold=0.78,
            sqqq_bear_strength_threshold=0.72,
            adx_threshold=20.0,
            max_episode_steps=120,
            cooldown_bars=45,
            drawdown_penalty_weight=0.12,
            reward_vol_normalization=True,
            score_edge_threshold=0.30,
        )
...
def main() -> None:
    ...
    base_env = DummyVecEnv([make_env(df)])

    sample_obs = base_env.reset()
    if isinstance(sample_obs, tuple):
        sample_obs = sample_obs[0]
    print(f"[train] env observation shape: {sample_obs.shape}")
    print(f"[train] env action space n: {base_env.action_space.n}")
    ...
    VECNORM_OUT.parent.mkdir(parents=True, exist_ok=True)
