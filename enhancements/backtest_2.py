...
class LiveTrader:
    """
    9-action runtime map aligned with the upgraded TradingEnv:
    ...
    Dollar-based backtest on a real portfolio.
    """
    ...
    # Safer sizing for leveraged ETFs
    TQQQ_ENTRY_ACTIONS = {1: 0.10, 2: 0.20, 3: 0.35}
    SQQQ_ENTRY_ACTIONS = {5: 0.10, 6: 0.20, 7: 0.35}
    ...
    def execute_action(self, action: int, row: pd.Series) -> str:
        ...
        if self._is_flat():
            if action in self.TQQQ_ENTRY_ACTIONS:
                ...
                # Disable transition entries for now
